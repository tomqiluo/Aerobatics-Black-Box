...
Code to put the Camera-Plane Bounding Box Pixel Coordinates for each frame into a CSV file
...
2. Bounding Boxes are rectangular planes
...
bl_info = {
    "name": "Export Bounding-Boxes To CSV file (BBOX ONLY)",
    "author": "Max Bakalos (& Ludvig Ström)",
    "version": (1, 2),
    "blender": (2, 80, 0),
    "location": "File > Export > Selected Animation Bbox (BBOX ONLY) (.csv)",
    "warning": 'Must be named "BoundBox_Object" & "Camera" in Blender',
    "description": "Selected object bounding box camera coords for each frame",
...
def write_some_data(context, filepath):
    ...
    for sel in selection:
        ...
        # Bounding-Box
        bbox = sel # Bounding-Boxes are selected
...
class xportTransformToCsvBbox_BBOX(Operator, ExportHelper):
    bl_idname = "export.transform_to_csv_bbox_bbox"  # important since its how bpy.ops.import_test.some_data is constructed
    bl_label = "Export Selected Animation Bbox (BBOX ONLY) (.csv)"
...
def menu_func_export(self, context):
    self.layout.operator(xportTransformToCsvBbox_BBOX.bl_idname, text="Selected Animation Bbox (BBOX ONLY) (.csv)")


def register():
    bpy.utils.register_class(xportTransformToCsvBbox_BBOX)
    bpy.types.TOPBAR_MT_file_export.append(menu_func_export)


def unregister():
    bpy.utils.unregister_class(xportTransformToCsvBbox_BBOX)
...
if __name__ == "__main__":
    ...
    # test call
    #bpy.ops.export.transform_to_csv_bbox_bbox('INVOKE_DEFAULT')
