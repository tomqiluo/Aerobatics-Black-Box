...
function ground_truth = blender_CSV_Image_2_matlab_GroundTruth_SkyBBOXES(im_folder_path, path_and_blender_csv_name, path_and_blender_skyBBOX_csv_name)
    ...
    %     structarray = s([], 1);
    %     sturctarray = s;
    %     structarray(dial_i).Position = [0,0,0,0];
    %     structarray(dial_i).Angle = 0;
    
        sky_structarray = sky_struct([]);
    %     sky_structarray = sky_struct([], 1);
    %     sky_structarray = sky_struct;
    %     sky_structarray(sky_i).Position = [0,0,0,0];
